test_that("connect and resume page exposes the staged workflow", {
  ...
  expected <- c(
    ...
    "connection-refresh_branches", "connection-new_review_branch",
    "connection-create_review_branch", "connection-available_branches"
  )
  expect_true(all(vapply(expected, grepl, logical(1), x = html, fixed = TRUE)))
})

test_that("connect page presents main as the protected review baseline", {
  html <- as.character(taggerUI:::.connect_resume_ui("connection"))
  expect_match(html, "published baseline in main is read-only", fixed = TRUE)
  expect_match(html, "Create review workspace", fixed = TRUE)
})
